(function () {
  ...
  var orderController = function (
    ...
    e.selectShippingService = function (service) {
      ...
      e.selectedShippingServiceIndex =
        e.avalibleShippingService.indexOf(service);
    ...
    e.deliveryFee = function () {
      if (e.subtotal() > 5000000) return 0;
      else return 250000;
    };
    ...
    e.getDistricts = function (provinceId) {
      if (provinceId !== undefined) {
        ...
        ghnServices.getDistrict(data).then(
          function (response) {
    ...
    e.getWards = function (districtId) {
      if (districtId !== undefined) {
        ...
        ghnServices.getWards(data).then(
          function (response) {
      ...
      authService
        ...
        .then(function (response) {
          ...
          if (e.addressCustomer.length !== 0)
            e.userInformation.phoneNumber =
              e.addressCustomer[e.selectedIndex].phoneNumber;
        })
        .catch(function (data) {
          console.log(data);
        });
      ...
      ghnServices.getAvalibleServices(data).then(
        function (response) {
          ...
          e.avalibleShippingService.forEach((item) => {
            ...
            ghnServices.getLeadTime(data).then(
              function (response) {
    ...
    e.getAvaliableServiceFeeData = function (service_type_id) {
      ...
      if (
        ...
        e.avalibleShippingService.length !== 0
      ) {
        data = {
    ...
    e.freeShip = function () {
      if (e.subtotal() > 5000000 && e.selectedShippingServiceIndex !== -1) {
        console.log(
          e.avalibleShippingService[e.selectedShippingServiceIndex].totalFee *
            -1
        );
        return (
          e.avalibleShippingService[e.selectedShippingServiceIndex].totalFee *
    ...
    e.createOrder = function (paymethod) {
      ...
      orderService.createOrder(data).then(
        function (response) {
          cartService.clearCart(tokenDecode.Id).then(
            function (response) {
    ...
    e.vnpay = function () {
      ...
      var urlPayRedirect =
        ...
        vnp_Locale +
        "&vnp_OrderInfo=" +
        encodeURIComponent(vnp_OrderInfo) +
        "&vnp_OrderType=" +
        vnp_OrderType +
        "&vnp_ReturnUrl=" +
        encodeURIComponent(vnp_ReturnUrl) +
        "&vnp_TmnCode=" +
        vnp_TmnCode +
        "&vnp_TxnRef=" +
        vnp_TxnRef +
    ...
    function constructor() {
      if (authService.isLoggedIn()) {
        ...
        cloudFlareService.getIPAddr().then(
          function (response) {
            e.cdn_cgi_trace = response.data
              .trim()
              .split("\n")
